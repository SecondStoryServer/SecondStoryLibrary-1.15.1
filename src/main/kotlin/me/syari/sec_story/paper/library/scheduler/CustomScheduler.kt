...
object CustomScheduler {
    ...
    fun runTimer(plugin: JavaPlugin, period: Long, delay: Long = 0, async: Boolean = false, run: CustomTask.() -> Unit): CustomTask? {
        return schedule(plugin, run).runTimer(period, delay, async)
    }

    fun runRepeatTimes(plugin: JavaPlugin, period: Long, times: Int, delay: Long = 0, async: Boolean = false, run: CustomTask.() -> Unit): CustomTask? {
